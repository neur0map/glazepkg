...
//
// These package-level vars are the single source of truth for every color used
// across the TUI.  They are initialised to the active theme at startup via
// ApplyTheme(), and re-assigned whenever the user cycles themes.  Because
// bubbletea's View() is called on every render, every style built from these
// vars automatically picks up the new colors on the very next frame.
...
// Rebuilt on every ApplyTheme call so badges always match the active palette.
var ManagerColors map[model.Source]lipgloss.Color

func rebuildManagerColors() {
	ManagerColors = map[model.Source]lipgloss.Color{
